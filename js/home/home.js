import ScrollButton from "./ButtonScroll.js";
  ...
  displayPhotographers(data) {
    ...
    photographers.map((photographe) => {
      ...
      articlePhotographers.className =
        photographe.tags.join(" ") + " photographer-profil";
      let templatePhotographer = `
            <a href="photographers.html?id=${photographe.id}" title="${
        photographe.name
      }">
                <img src="${
                  photographe.portrait
                }" alt="${photographe.name} profil picture">
                <h2 class="name">${photographe.name}</h2>
            </a>
            <p class="location">${photographe.city}, ${photographe.country}</p>
            <p class="tagline">${photographe.tagline}</p>
            <p class="price">${photographe.price}€/jour</p>
            <ul class="phtags">${photographe.tags
              .map((tag) => `<li class="tag" data-filter="${tag}">#${tag}</li>`)
              .join(" ")}</ul> 
            `;
    ...
    new ScrollButton().displayButton();
    new filterTags().init();
  }
}
